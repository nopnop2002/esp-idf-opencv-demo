...
esp_err_t camera_capture(char * filterName, char * sourceFileName, char * opencvFileName, int crop_size, int crop_position)
{
	...
	// Crop image
	cv_crop_image(fb->buf, fb->len, fb->width, fb->height, crop_size, crop_position);
...
#define SHUTTER "Keybord Enter"
	...
	HTTP_t httpBuf;
	strcpy(httpBuf.filterName, "Crop Image");
	...
	while(1) {
		...
		for (int i=0;i<commandNum;i++) {
			...
			if (cmdBuf.command == commands[i]) crop_position = i;
		}
		ESP_LOGI(TAG, "crop_position=%d", crop_position);
		if (crop_position < 0) continue;
		...
		while(1) {
			...
			camera_capture(httpBuf.filterName, httpBuf.sourceFileName, httpBuf.opencvFileName, 100, crop_position);
			//listSPIFFS("/spiffs/");
			if (stat(httpBuf.sourceFileName, &stat1) == 0) {
				if (stat(httpBuf.opencvFileName, &stat2) == 0) {
