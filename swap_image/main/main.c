...
esp_err_t camera_capture(char * filterName, char * sourceFileName, char * opencvFileName, int swap_size, int swap_position)
{
	...
	// Swap image
	cv_swap_image(fb->buf, fb->len, fb->width, fb->height, swap_size, swap_position);
...
void app_main(void)
{
	...
	// Function table
	uint16_t commands[] = {'0', '1', '2', '3'};
	char *positions[] = {
		"Swap the top left and top right corners",
		"Swap the middle left and middle right corners",
		"Swap the bottom left and bottom right corners",
		"Swap the top center and bottom center corners"
	};
...
#define SHUTTER "Keybord Enter"
	...
	HTTP_t httpBuf;
	strcpy(httpBuf.filterName, "Swap Image");
	...
	while(1) {
		...
		for (int i=0;i<commandNum;i++) {
			...
			if (cmdBuf.command == commands[i]) swap_position = i;
		}
		ESP_LOGI(TAG, "swap_position=%d", swap_position);
		if (swap_position < 0) continue;
		...
		while(1) {
			...
			camera_capture(httpBuf.filterName, httpBuf.sourceFileName, httpBuf.opencvFileName, 100, swap_position);
			//listSPIFFS("/spiffs/");
			if (stat(httpBuf.sourceFileName, &stat1) == 0) {
				if (stat(httpBuf.opencvFileName, &stat2) == 0) {
