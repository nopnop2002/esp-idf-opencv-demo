...
static esp_err_t root_get_handler(httpd_req_t *req)
{
	...
	if (stat(sourceFileName, &st) != 0) {
		ESP_LOGE(TAG, "[%s] not found", sourceFileName);
		httpd_resp_sendstr_chunk(req, NULL);
		return ESP_OK;
	}
	ESP_LOGI(TAG, "%s exist st.st_size=%ld", sourceFileName, st.st_size);
	...
	if (img_src_buffer == NULL) {
		...
		esp_err_t ret = Image2Base64(sourceFileName, st.st_size, img_src_buffer, img_src_buffer_len);
	...
	httpd_resp_sendstr_chunk(req, filterName);
	httpd_resp_sendstr_chunk(req, "<br>");

	// Put opencv file
	if (stat(opencvFileName, &st) != 0) {
		ESP_LOGE(TAG, "[%s] not found", opencvFileName);
		httpd_resp_sendstr_chunk(req, NULL);
		return ESP_OK;
	}
	ESP_LOGI(TAG, "%s exist st.st_size=%ld", opencvFileName, st.st_size);
	base64Size = calcBase64EncodedSize(st.st_size);
	ESP_LOGI(TAG, "base64Size=%"PRIi32, base64Size);

	// Convert from JPEG to BASE64
	img_src_buffer = NULL;
	img_src_buffer_len = base64Size + 1;
	img_src_buffer = malloc(img_src_buffer_len);
	if (img_src_buffer == NULL) {
		ESP_LOGE(TAG, "malloc fail. img_src_buffer_len %d", img_src_buffer_len);
	} else {
		esp_err_t ret = Image2Base64(opencvFileName, st.st_size, img_src_buffer, img_src_buffer_len);
		ESP_LOGI(TAG, "Image2Base64=%d", ret);
		if (ret != 0) {
			ESP_LOGE(TAG, "Error in mbedtls encode! ret = -0x%x", -ret);
		} else {
...
void http_task(void *pvParameters)
{
	...
	while(1) {
		...
		if (xQueueReceive(xQueueHttp, &httpBuf, portMAX_DELAY) == pdPASS) {
			ESP_LOGI(TAG, "httpBuf.sourceFileName=[%s]", httpBuf.sourceFileName);
			filterName = httpBuf.filterName;
			sourceFileName = httpBuf.sourceFileName;
			opencvFileName = httpBuf.opencvFileName;
