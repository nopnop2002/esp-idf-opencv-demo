...
extern "C" void cv_text_detection(uint8_t* data, int dlen, int width, int heigth);
...
void cv_text_detection(uint8_t* data, int dlen, int width, int heigth)
{
    printf("OpenCV text detection.\n");
    cv::Mat small( heigth, width, CV_8UC1, (void*)data );
	cv::Mat cloned = small.clone();
    int cols = small.cols;
    int rows = small.rows;
    int channels = small.channels();
    int elemSize = small.elemSize();
    ...
    for(int idx = 0; idx >= 0; idx = hierarchy[idx][0]){
        ...
        {
			for (int i = 0; i < 4; i++)
				line(cloned, pts[i], pts[(i+1)%4], cv::Scalar(0,255,0));
