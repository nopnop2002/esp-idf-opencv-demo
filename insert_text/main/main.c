...
esp_err_t camera_capture(char * filterName, char * sourceFileName, char * opencvFileName, uint16_t color)
{
	...
	// Insert text
	cv_insert_text(fb->buf, fb->len, fb->width, fb->height, color);
...
void app_main(void)
{
	...
	// Function table
	uint16_t commands[] = {'b', 'r', 'g', 'y', 'c', 'p', 'w', 'z'};
	char *colors[] = {
		"Blue",
		"Red",
		"Green",
...
#define SHUTTER "Keybord Enter"
	...
	HTTP_t httpBuf;
	strcpy(httpBuf.filterName, "Insert Text");
	...
	while(1) {
		...
		for (int i=0;i<commandNum;i++) {
			...
			if (cmdBuf.command == commands[i]) index = i;
		}
		ESP_LOGI(TAG, "index=%d", index);
		if (index < 0) continue;
		uint16_t color = RGB888_TO_16UC1(rgb[index][0], rgb[index][1], rgb[index][2]);
		...
		while(1) {
			...
			camera_capture(httpBuf.filterName, httpBuf.sourceFileName, httpBuf.opencvFileName, color);
			//listSPIFFS("/spiffs/");
			if (stat(httpBuf.sourceFileName, &stat1) == 0) {
				if (stat(httpBuf.opencvFileName, &stat2) == 0) {
