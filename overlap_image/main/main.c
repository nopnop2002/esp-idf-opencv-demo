...
static camera_config_t camera_config = {
	...
	//.pixel_format = PIXFORMAT_YUV422,//YUV422,PIXFORMAT_GRAYSCALE,RGB565,PIXFORMAT_JPEG
	.pixel_format = PIXFORMAT_RGB565,//YUV422,PIXFORMAT_GRAYSCALE,RGB565,PIXFORMAT_JPEG
	.frame_size = FRAMESIZE_HVGA, //QQVGA-UXGA Do not use sizes above QVGA when not JPEG
	//.frame_size = FRAMESIZE_128X128, //QQVGA-UXGA Do not use sizes above QVGA when not JPEG
	...
	uint8_t * overlay_data, int overlay_width, int overlay_heigth, int overlay_position)
{
	...
	// Overlay image
	cv_overlay_image(fb->buf, fb->len, fb->width, fb->height, overlay_data, overlay_width, overlay_heigth, overlay_position);

	// Convert camera frame buffer to JPEG
	// jpg_buf needs to be released
	jpeg_converted = frame2jpg(fb, 20, &jpg_buf, &jpg_buf_len);
	if(!jpeg_converted){
		ESP_LOGE(TAG, "JPEG compression failed");
		esp_camera_fb_return(fb);
		return ESP_FAIL;
...
void app_main(void)
{
	...
	// Read overlap image
	char overlapFileName[64];
	sprintf(overlapFileName, "%s/image.raw", base_path);
	struct stat statInsert;
	if (stat(overlapFileName, &statInsert) != 0) {
		ESP_LOGE(TAG, "overlap file not exist");
		while(1) { vTaskDelay(100); }
	}
	size_t fsize = statInsert.st_size;
	// fsize = width * height * 2
	int overlay_width = sqrt((double)fsize / 2.0);
	// Overwrap images must be square
	int overlay_height = overlay_width;
	ESP_LOGI(TAG, "fsize=%d overlay_width=%d", fsize, overlay_width);

	uint8_t *overlap_image_buffer = NULL;
	overlap_image_buffer = malloc(fsize);
	if (overlap_image_buffer == NULL) {
		ESP_LOGE(TAG, "malloc fail. overlap_image_buffer %d", statInsert.st_size);
		while(1) { vTaskDelay(100); }
	}
	FILE * fp;
	if((fp=fopen(overlapFileName,"rb"))==NULL){
		ESP_LOGE(TAG, "fopen fail. [%s]", overlapFileName);
		while(1) { vTaskDelay(100); }
	}else{
		for (int i=0;i<fsize;i++) {
			fread(&overlap_image_buffer[i],sizeof(char),1,fp);
		}
		fclose(fp);
	}
	...
	// Function table
	uint16_t commands[] = {'0', '1', '2', '3', '4', '5', '6', '7', '8'};
	char *positions[] = {
		"top left corner",
		"top center",
		"top right corner",
...
#define SHUTTER "Keybord Enter"
	...
	HTTP_t httpBuf;
	strcpy(httpBuf.filterName, "Overlap Image");
	...
	while(1) {
		...
		for (int i=0;i<commandNum;i++) {
			printf("%c:%s\n", commands[i], positions[i]);
		}
		ESP_LOGW(TAG,"Waitting %s ....", SHUTTER);
		xQueueReceive(xQueueCmd, &cmdBuf, portMAX_DELAY);
		ESP_LOGI(TAG,"cmdBuf.command=%x[%c]", cmdBuf.command, cmdBuf.command);
		int overlap_position = -1;
		for (int i=0;i<commandNum;i++) {
			ESP_LOGD(TAG, "commands[i]=%x", commands[i]);
			if (cmdBuf.command == commands[i]) overlap_position = i;
		}
		ESP_LOGI(TAG, "overlap_position=%d", overlap_position);
		if (overlap_position < 0) continue;
		...
		while(1) {
			...
			camera_capture(httpBuf.filterName, httpBuf.sourceFileName, httpBuf.opencvFileName, 
				overlap_image_buffer, overlay_width, overlay_height, overlap_position);
			//listSPIFFS("/spiffs/");
			if (stat(httpBuf.sourceFileName, &stat1) == 0) {
				if (stat(httpBuf.opencvFileName, &stat2) == 0) {
