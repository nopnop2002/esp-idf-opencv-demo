...
extern "C" void cv_swap_image(uint8_t* data, int dlen, int width, int heigth, int swp_size, int swap_position);
...
void cv_swap_image(uint8_t* data, int dlen, int width, int heigth, int swp_size, int swap_position)
{
	...
	int swap_x1 = 0;
	int swap_y1 = 0;
	int swap_x2 = src.cols - swp_size;
	int swap_y2 = swap_y1;

	// Swap position(Top left X coordinate, Y coordinate)
	if (swap_position == 1) {
		swap_y1 = (rows / 2) - (swp_size / 2);
		swap_y2 = swap_y1;
	} else if (swap_position == 2) {
		swap_y1 = rows - swp_size;
		swap_y2 = swap_y1;
	} else if (swap_position == 3) {
		swap_x1 = (cols / 2) - (swp_size / 2);
		swap_x2 = swap_x1;
		swap_y2 = rows - swp_size;
	}

	// Crop image
	cv::Rect roi1 = cv::Rect(swap_x1, swap_y1, swp_size, swp_size); 
	cv::Mat image_roi1 = cloned(roi1);
	cv::Rect roi2 = cv::Rect(swap_x2, swap_y2, swp_size, swp_size); 
